...
2. **获取缓存**：在 jeesite skills 根目录下执行 `python3 scripts/cache_docs.py --skill quick-start --permalink <permalink>` 脚本，缓存目录为 `references/.cache/quick-start/`，不存在时自动回退到 `~/.cache/jeesite/quick-start/`
