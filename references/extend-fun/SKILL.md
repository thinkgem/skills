...
2. **获取缓存**：在 jeesite skills 根目录下执行 `python3 scripts/cache_docs.py --skill extend-fun --permalink <permalink>` 脚本，缓存目录为 `references/.cache/extend-fun/`，不存在时自动回退到 `~/.cache/jeesite/extend-fun/`
3. **读取内容**：先读 `__00.md` 目录索引文件，了解章节结构，按需读取 `__01.md`~`__NN.md`
4. **回答问题**：缓存文件有效期3天，自动更新缓存，并基于读取的文档内容回答

## 关键词

BPM、工作流、Flowable、CMS、内容管理、AI、消息推送、SSO、单点登录、OAuth2、作业调度、对象存储、OSS、可视化、报表、文件管理、文件预览、UniApp、手机端移动端、三员管理、RAG、知识库、智能助手、CAS、Quartz、S3、ECharts、UReport、认证服务器、流程设计、审批、会签、退回、加签、站内信、消息提醒、定时任务、大屏、文件柜、在线预览、App、敏感词、敏感词过滤、敏感词验证、敏感词查询
...
| 敏感词过滤 | 敏感词库管理 | /sensitive-word/ | 基于DFA算法的敏感词过滤、验证、查询、替换。关键章节：接口调用、后端工具 | |
...
| 敏感词过滤怎么用？ | 敏感词过滤 | https://jeesite.com/docs/sensitive-word/ |
